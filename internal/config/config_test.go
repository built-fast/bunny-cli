...
func TestDir_FallsBackToHome(t *testing.T) {
	...
	got := Dir()
	want := filepath.Join(home, ".config", "bunny")
	if got != want {
		t.Errorf("Dir() = %q, want %q", got, want)
	}
}
...
func TestFilePath(t *testing.T) {
	t.Setenv("XDG_CONFIG_HOME", "/custom/config")
	got := FilePath()
	want := "/custom/config/bunny/config.toml"
	if got != want {
...
func TestWrite_PreservesExistingKeys(t *testing.T) {
	...
	if err := Write("storage_key", "storage-pass"); err != nil {
		t.Fatalf("Write(storage_key) error: %v", err)
	}
	...
	if got := v.GetString("storage_key"); got != "storage-pass" {
		t.Errorf("storage_key = %q, want %q", got, "storage-pass")
	}
}
