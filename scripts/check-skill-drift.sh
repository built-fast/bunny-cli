...
#
# Three checks:
# 1. Every `bunny <cmd>` in SKILL.md resolves to a CMD in .surface
# 2. Every --<flag> on a line with `bunny <cmd>` exists on the resolved
...
done < <(echo "$skill_content" | grep 'bunny [a-z]' | grep -- '--[a-z]')
