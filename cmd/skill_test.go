...
func TestRunSkillInstall_FileCreation(t *testing.T) {
	...
	dest := filepath.Join(cfg.agentsDir, "bunny", "SKILL.md")
	...
	// Verify .version file
	versionContent, err := os.ReadFile(filepath.Join(cfg.agentsDir, "bunny", ".version"))
	if err != nil {
	...
	if !strings.Contains(output, "bunny configure") {
		t.Error("missing next steps")
	}

	// Verify symlink in claude dir
	link := filepath.Join(cfg.claudeDir, "bunny", "SKILL.md")
...
func TestRunSkillInstall_CopyFallback(t *testing.T) {
	...
	agentFile := filepath.Join(cfg.agentsDir, "bunny", "SKILL.md")
...
func TestRunSkillUninstall_RemovesFiles(t *testing.T) {
	...
	agentsDir := filepath.Join(cfg.agentsDir, "bunny")
	claudeDir := filepath.Join(cfg.claudeDir, "bunny")
	...
	if _, err := os.Stat(agentsDir); !os.IsNotExist(err) {
		t.Errorf("agents bunny dir still exists after uninstall")
	}
	if _, err := os.Stat(claudeDir); !os.IsNotExist(err) {
		t.Errorf("claude bunny dir still exists after uninstall")
	}
...
func TestSkillPathCmd(t *testing.T) {
	t.Parallel()

	out, _, err := executeCommand(nil, "skill", "path")
	if err != nil {
		t.Fatalf("skill path command failed: %v", err)
	}

	trimmed := strings.TrimSpace(out)
	if !strings.HasSuffix(trimmed, filepath.Join(".agents", "skills", "bunny")) {
		t.Errorf("skill path output = %q, expected path ending in .agents/skills/bunny", trimmed)
	}
}
