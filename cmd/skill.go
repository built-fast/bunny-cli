...
import (
	...
	"github.com/built-fast/bunny-cli/skills"
...
func newSkillPathCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "path",
		Short: "Print the skill installation path",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := defaultInstallConfig()
			if err != nil {
				return err
			}
			_, err = fmt.Fprintln(cmd.OutOrStdout(), filepath.Join(cfg.agentsDir, "bunny"))
			return err
		},
	}
}
...
func refreshSkills(cfg *installConfig) {
	...
	versionFile := filepath.Join(cfg.agentsDir, "bunny", ".version")
...
func runSkillUninstall(w io.Writer, cfg *installConfig) error {
	// Remove ~/.agents/skills/bunny/
	agentsBunny := filepath.Join(cfg.agentsDir, "bunny")
	if err := os.RemoveAll(agentsBunny); err != nil {
		return fmt.Errorf("removing %s: %w", agentsBunny, err)
	}

	// Remove ~/.claude/skills/bunny/
	claudeBunny := filepath.Join(cfg.claudeDir, "bunny")
	if err := os.RemoveAll(claudeBunny); err != nil {
		return fmt.Errorf("removing %s: %w", claudeBunny, err)
	}

	_, _ = fmt.Fprintln(w, "Uninstalled bunny skill files.")
	return nil
}
...
func runSkillInstall(w io.Writer, cfg *installConfig) error {
	...
	// 1. Write to ~/.agents/skills/bunny/SKILL.md
	destDir := filepath.Join(cfg.agentsDir, "bunny")
	...
	// 3. Symlink from ~/.claude/skills/bunny/SKILL.md → installed file
	linkDir := filepath.Join(cfg.claudeDir, "bunny")
	...
	_, _ = fmt.Fprintln(w, "  Run `bunny configure` to set up authentication")
